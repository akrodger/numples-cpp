...
int main(int argc, char** argv){
	...
	do{
		currGrad = J.grad();
		xOpt = wolfeLineSearch(	J,//Objective& J,
								currGrad*-1,//const Matrix& searchDirect,
								1e-4,//double c1,
								0.9,//double c2,
								1e-4,//double alphaMax
								stopSignal);
		J.update(xOpt);
		numIter++;
	}while(currGrad.norm2() >= 1e-10 && (stopSignal == 0));
	printf("\nSteepest Descent Objective Value: %le\n", J.getValue());
	xOpt.print();
	currGrad.print();
	printf(	"\nStop signal was : %d\n"
			  "Iterations was  : %ld\n", stopSignal, numIter);
